(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('rethinkdbjhipsterApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-rethinkdbjhipsterApp-error');
                var entityKey = httpResponse.headers('X-rethinkdbjhipsterApp-params');
                if (errorHeader) {
